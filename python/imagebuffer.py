...
class ImageBuffer(object):
	...
	def insert(self,image_num):
		...
		with self.lock: #used to remove possible race condition
			...
			if not self.not_buffering.is_set() and self.queue_size.value>= self.max_buffer_size: #wait till queue_size > = init_queue_size
	...
	def next(self):
		...
		with self.lock: #adds consistent view of the queue size
			print "DEBUG: IMAGE QUEUE SIZE:", str(self.queue_size.value)
			if self.queue_size.value < self.min_buffer_size: #if image queue falls below a certain len (block)
				self.not_buffering.clear()

		print "DEBUG: WAITING FOR BUFFERING"
		self.not_buffering.wait() #buffering
                print "DEBUG: NO BUFFERING"
	...
	def calculate_consumption_rate(self,production_rate):
		...
		"""
		if self.queue_size.value <= self.mid_buffer_size:
			self.consumption_rate = production_rate+(self.mid_buffer_size+1-self.queue_size.value)*.1*production_rate
			return self.consumption_rate if self.consumption_rate>0 else production_rate
		elif self.queue_size.value>= self.max_buffer_size:
			self.consumption_rate = production_rate-(self.queue_size.value+1-self.max_buffer_size)*.5*production_rate
			return self.consumption_rate if self.consumption_rate>0 else production_rate
		else:
