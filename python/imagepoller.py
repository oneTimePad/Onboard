...
'''
This class is responsible for keeping track of the filepath of the next image to be posted to the ground station and 
whether or not it is ready to be posted
'''
...
class ImagePoller(object):
    ...
    def __init__(self, next_image_number=1, image_poll_directory):
        ...
        self.next_image_filepath = image_poll_directory + "/capt" + str(next_image_number) + ".jpeg"
        
        
        
    # returns True if the next image is ready to be posted, returns False otherwise
    def next_image_isready(self):
        return os.path.isfile(next_image_filepath)
        
    # exposes next_image_filepath
    def get_next_image_filepath(self):
        return current_image_filepath
    
    # incrememnts current_image_number and recomputes current_image_filepath
    def increment(self):
