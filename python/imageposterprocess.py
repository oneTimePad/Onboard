...
if len(sys.argv) != 10:
    print("Error - Incorrect number of command line arguments passed to imageposterprocess.py.\n" 
          "Usage: python imageposterprocess.py [next_image_number] [image_poll_directory] [telemetry_poll_directory] [server_ip] [server_port] [username] [password] [access_token_as_text] [sleep_time]")
    sys.exit(1)
next_image_number = int(sys.argv[1])
image_poll_directory = sys.argv[2]
telemetry_poll_directory = sys.argv[3]
server_ip = sys.argv[4]
server_port = sys.argv[5]
username = sys.argv[6]
password = sys.argv[7]
access_token_as_text = sys.argv[8]
sleep_time = int(sys.argv[9])


# initialize the ImagePoller and DroneAPI objects
image_poller = ImagePoller(next_image_number, image_poll_directory, telemetry_poll_directory)
